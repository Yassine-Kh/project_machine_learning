...
def bestParameters(X_train, X_test, y_train, y_test, kernels,gammas,tols):
        ...
        bestpred = SVM(kernel=svmModels[max_index][0], gamma=svmModels[max_index][1],
                       tol=svmModels[max_index][2]).fitAndPredict(X_train, X_test, y_train)
        print("Classification report :\n", classification_report(y_test, bestpred))
        return f"The SVM model with the best accuracy has the parameters: kernel: {svmModels[max_index][0]}, " \
               f"gamma: {svmModels[max_index][1]}, tol {svmModels[max_index][2]}, " \
